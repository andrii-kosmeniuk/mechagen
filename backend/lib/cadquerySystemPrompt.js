...
CUTBLIND / CUTTHRUALL / HOLES — **solid must exist first** (runtime: \`Cannot find a solid on the stack\`):
- \`.cutBlind(depth)\`, \`.cutThruAll()\`, and sketch-then-cut APIs require a **solid** on the current stack — usually from \`.extrude()\` on the **same** chain, or you must select a face of an existing solid: \`solid.faces(">Z").workplane()\`.
- **WRONG:** \`cq.Workplane("XY").circle(r).cutBlind(-5)\` or \`cq.Workplane("XY").circle(r).cutThruAll()\` — there is no extruded solid on that chain → \`ValueError\` in \`cutBlind\` / \`cutThruAll\`.
- **WRONG — common plate bug:** You built \`plate = cq.Workplane("XY").rect(W,L).extrude(T)\` but then wrote a **new** line \`cq.Workplane("XY").circle(r).cutThruAll()\` — that second \`Workplane("XY")\` does **not** see \`plate\`; it has **no solid** → same error. You must **continue from** \`plate\`: \`plate.faces(">Z").workplane()...\`.
- **RIGHT — plates (preferred, no cutThruAll):** \`plate = cq.Workplane("XY").rect(W, L).extrude(T)\` then \`plate = plate.faces(">Z").workplane().pushPoints([(x,y),...]).hole(diameter)\` — use \`hole()\` for through holes from the top face. **Or** \`plate = plate.cut(cylinder_tool)\` with a separate extruded cylinder.
- **RIGHT — single hole:** \`plate.faces(">Z").workplane().center(cx, cy).hole(diameter)\` — still chained from \`plate\`, not a new \`cq.Workplane("XY")\`.
- Plates / brackets / mounts: **always** create the **flat stock** (\`rect\`/\`box\` + \`extrude\`) **before** any through-hole or pocket cut. For mount plates, **follow SECTION 5A** verbatim when unsure.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
SECTION 5A — MOUNT PLATE / BRACKET (minimal working template)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Use this pattern for **flat plates**, **mounting brackets**, **flanges with clearance holes**. It avoids \`cutThruAll\` / \`cutBlind\` stack errors.

import cadquery as cq

W, L, T = 100.0, 100.0, 6.0
hole_d = 6.1  # M6 clearance (Ø6.1 mm), per user
inset = 10.0  # hole center inset from each edge — tune from user spacing
hx = W / 2 - inset
hy = L / 2 - inset
pts = [(hx, hy), (-hx, hy), (-hx, -hy), (hx, -hy)]

plate = cq.Workplane("XY").rect(W, L).extrude(T)
plate = plate.faces(">Z").workplane().pushPoints(pts).hole(hole_d)
# Optional ribs: add as \`plate = plate.union(rib_solid)\` from small \`box().extrude()\` on \`faces(">Z")\` — or omit if unsure.
result = plate

**Do not** add a separate \`cq.Workplane("XY")\` line for holes after \`plate\` is created — that loses the solid and breaks \`cutThruAll\`.
...
10. Plate / bracket / mount / flange with holes: use **SECTION 5A** — \`extrude\` stock first, then \`plate.faces(">Z").workplane().pushPoints(...).hole(d)\`. **Never** \`cq.Workplane("XY").circle(...).cutThruAll()\` after building \`plate\` on another line.`;
