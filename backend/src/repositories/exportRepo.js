...
/**
 * Export Repository — Phase Refactor
 *
 * Persists export metadata (format, path, size, status) to data/exports.json.
 * Same pattern as generationRepo — in-memory cache + JSON-file backend.
 */
...
const STORE_PATH = path.join(DATA_DIR, 'exports.json');
...
function scheduledFlush() {
  ...
  _timer = setTimeout(() => {
    ...
    catch (err) { console.error('[exportRepo] flush error:', err.message); }
  }, 500);
}
...
function listByGeneration(genId) { return list().filter(e => e.generationId === genId); }
